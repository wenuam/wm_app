...
* :file_folder: `%AppData%`: user files (Roaming)
	* :file_folder: **wm_app**
		* :file_folder: **repository** (complete name starting with "wm_app")
			* :file_folder: **log**
				* :page_with_curl: The `application-[version|tag]-date_time.[stdout|stderr|hubfs].log` files...
...
Relative paths (starting with `.`/`..`) are resolved from the `batch`/`bash` file's location.
...
When I say **proper** I mean "fast", "efficient", "lightweight", "readable", "maintainable", "long-lasting", not random engineering aberrations "fixing" dummy Kanban tickets to please clueless bosses. As the proverb goes [“if it ain't broke, don't fix it”](https://www.youtube.com/watch?v=YjYz3EQ5fOo)...
